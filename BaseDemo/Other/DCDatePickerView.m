...
@interface DCDatePickerView()<UIPickerViewDataSource, UIPickerViewDelegate>
/** 选择的年 */
...
{
    if (self = [super init]) {
        self.isAddYetSelect = isAddYetSelect;
        ...
        [self setDefaultTSelectYear:[dataCom year] defaultSelectMonth:[dataCom month] defaultSelectDay:[dataCom day]];
...
{
    NSString *timeString = [NSString stringWithFormat:@"%@-%@-%@",[NSString stringWithFormat:@"%ld",self.selectYear],self.selectMonth<10?[NSString stringWithFormat:@"0%ld",self.selectMonth]:[NSString stringWithFormat:@"%ld",self.selectMonth],self.selectDay<10?[NSString stringWithFormat:@"0%ld",self.selectDay]:[NSString stringWithFormat:@"%ld",self.selectDay]];
    if (_block) {
        _block(timeString);
    }
